...
% Αρχική ακολουθία x[n]
L = 10; n = [0:1:L]; x = 0.9 .^ n;

% Κυκλική μετατόπιση y[n] = x[[n-m] mod N]
n0 = 8;  % Μετατόπιση σημείων
N = 15; % Μήκος ακολουθίας
x = [x zeros(1, N-L-1)];
n = [0:1:N-1]; y = x(mod(n-n0, N)+1);

% Σχεδίαση ακολουθιών x[n] και y[n]
figure(1); 
subplot(121); stem(n,x); title('Αρχική ακολουθία x[n]'); grid on
...
figure(2); 
subplot(221); stem(k,abs(X)); title('Μέτρο |X[k]|'); grid on
