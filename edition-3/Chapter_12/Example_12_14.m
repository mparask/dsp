...
figure(1); subplot(131); stem(n,x); title('Αρχική ακολουθία x[n]'); grid on
...
figure(2); subplot(221); stem(k, real(Xce)); 
title('Πραγματικό μέρος Xce[k]'); grid on
subplot(222); stem(k, real(X)); 
title('Πραγματικό μέρος X[k]'); grid on

% Σχεδίαση φανταστικού μέρους DFT Xco[k] X[k]
subplot(223); stem(k, imag(Xco)); 
title('Φανταστικό μέρος Xco[k]'); grid on
subplot(224); stem(k, imag(X)); 
title('Φανταστικό μέρος X[k]'); grid on